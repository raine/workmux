...
Press `t` to switch between grouped and flat, or use the command line:
...
Which groups are open is shared by every sidebar pane and survives a tmux
restart. A collapsed group still lists the agent of the window its sidebar sits
in, so the sidebar never hides where you are, and the toggle counts only what
stays folded.

Folded agents are not jump targets: `{idx}` and `{jump_key}` skip them,
`workmux sidebar next|prev|jump <N>` passes over them, and the numbers close up
rather than leaving gaps. Unfold the group and its agents take their numbers
back.
