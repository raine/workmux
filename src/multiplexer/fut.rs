//! Fut backend implementation for the `Multiplexer` contract.
//!
//! Fut exposes its persistent server through the `fut` CLI.  Commands below
...
pub struct FutBackend;

impl FutBackend {
    pub fn new() -> Self { Self }

    fn run(&self, args: &[String]) -> Result<String> {
        let output = Command::new("fut").args(args).output()
            .with_context(|| format!("failed to execute fut {}", args.join(" ")))?;
        if !output.status.success() {
            return Err(anyhow!("fut {} failed: {}", args.join(" "), String::from_utf8_lossy(&output.stderr).trim()));
        }
    ...
    fn json(&self, args: &[String]) -> Result<Value> {
        ...
        serde_json::from_str(&output).with_context(|| format!("fut returned non-JSON for {}", args.join(" ")))
    }
    ...
    fn pane_list(&self) -> Result<Vec<Value>> { Ok(Self::items(self.json(&["list".into(), "--json".into()])?)) }
    fn tab_list(&self) -> Result<Vec<Value>> { Ok(Self::items(self.json(&["list".into(), "--json".into()])?)) }
    fn focused_workspace(&self) -> Result<String> {
        self.items(self.json(&["list".into(), "--json".into()])?).into_iter()
            .find(|item| item.get("focused").and_then(Value::as_bool).unwrap_or(false))
            .and_then(|item| Self::id(&item))
            .ok_or_else(|| anyhow!("fut has no focused workspace"))
    }
    fn focused_tab(&self) -> Result<String> {
        self.tab_list()?.into_iter().find(|item| item.get("focused").and_then(Value::as_bool).unwrap_or(false))
            .and_then(|item| Self::id(&item)).ok_or_else(|| anyhow!("fut has no focused tab"))
    }
...
impl Multiplexer for FutBackend {
    fn name(&self) -> &'static str { "fut" }
    fn is_running(&self) -> Result<bool> { Ok(self.run(&["list".into(), "--json".into()]).is_ok()) }
    fn current_pane_id(&self) -> Option<String> { std::env::var("FUT_PANE_ID").ok() }
    ...
    fn get_client_active_pane_path(&self) -> Result<PathBuf> {
        let pane = self.active_pane_id().ok_or_else(|| anyhow!("no active fut pane"))?;
        let value = self.pane_by_id(&pane)?.ok_or_else(|| anyhow!("fut pane {pane} not found"))?;
        value.get("cwd").or_else(|| value.get("working_dir")).and_then(Value::as_str).map(PathBuf::from).ok_or_else(|| anyhow!("fut did not report pane cwd"))
    }
    fn create_window(&self, params: CreateWindowParams) -> Result<String> {
        ...
        let mut args = vec!["tab".into(), "new".into(), workspace, "--cwd".into(), cwd, "--name".into(), name, "--json".into()];
        self.json(&args)?;
        self.pane_list()?.into_iter().find_map(|pane| { let id = Self::id(&pane)?; (!before.iter().any(|old| Self::id(old).as_deref() == Some(&id))).then_some(id) }).ok_or_else(|| anyhow!("fut did not return a pane for the new tab"))
    }
    fn create_session(&self, _params: CreateSessionParams) -> Result<String> { Err(anyhow!("session mode is not supported by fut; use workspace/window mode")) }
    fn switch_to_session(&self, _prefix: &str, _name: &str) -> Result<()> { Err(anyhow!("session mode is not supported by fut")) }
    ...
    fn schedule_window_close(&self, full_name: &str, delay: Duration) -> Result<()> { self.run_deferred_script(&format!("sleep {}; fut tab close '{}'", delay.as_secs_f64(), full_name.replace('\'', "'\\''"))) }
    fn schedule_session_close(&self, _full_name: &str, _delay: Duration) -> Result<()> { Err(anyhow!("session mode is not supported by fut")) }
    fn run_deferred_script(&self, script: &str) -> Result<()> { util::run_detached_sh_c(script) }
    fn shell_select_window_cmd(&self, full_name: &str) -> Result<String> { let tab = self.tab_by_name(full_name)?.ok_or_else(|| anyhow!("tab not found"))?; Ok(format!("fut tab focus {}", super::agent::shell_quote(&Self::id(&tab).unwrap()))) }
    fn shell_kill_window_cmd(&self, full_name: &str) -> Result<String> { Ok(format!("fut tab close {}", super::agent::shell_quote(full_name))) }
    fn shell_switch_session_cmd(&self, _full_name: &str) -> Result<String> { Err(anyhow!("session mode is not supported by fut")) }
    fn shell_kill_session_cmd(&self, _full_name: &str) -> Result<String> { Err(anyhow!("session mode is not supported by fut")) }
    ...
    fn wait_until_session_closed(&self, _full_session_name: &str) -> Result<()> { Err(anyhow!("session mode is not supported by fut")) }
    ...
    fn split_pane(&self, target: &str, direction: &SplitDirection, cwd: &Path, _size: Option<u16>, _percentage: Option<u8>, command: Option<&str>) -> Result<String> {
        let direction = match direction { SplitDirection::Horizontal => "right", SplitDirection::Vertical => "down", SplitDirection::Stacked => return Err(anyhow!("fut does not support stacked splits")) };
        let before: HashSet<_> = self.pane_list()?.into_iter().filter_map(|p| Self::id(&p)).collect();
        self.run(&["pane".into(), "new".into(), self.focused_tab()?, "--cwd".into(), Self::path_arg(cwd)?, "--json".into()])?;
        let pane = self.pane_list()?.into_iter().find_map(|p| { let id = Self::id(&p)?; (!before.contains(&id)).then_some(id) }).ok_or_else(|| anyhow!("fut did not report new split pane"))?;
        if let Some(command) = command { self.run(&["terminal".into(), "report".into(), pane.clone(), command.into()])?; }
        Ok(pane)
    }
    fn instance_id(&self) -> String { std::env::var("FUT_SOCKET").unwrap_or_else(|_| "default".into()) }
